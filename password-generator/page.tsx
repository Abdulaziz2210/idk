...
import { Copy, RefreshCw, Send, Shield, Eye, EyeOff, Trash2 } from "lucide-react"
...
*Admin Note:* Password sent via secure channel.
      ...
      if (data.success) {
        ...
        const userIndex = registeredUsers.findIndex((user) => user.fullName.toLowerCase() === studentName.toLowerCase())
        ...
        if (userIndex >= 0) {
          ...
          updatedUsers[userIndex] = {
            ...user,
            password: generatedPassword, // Store password as-is
          ...
          setError(`User ${studentName} not found in registered users`)
        }
  ...
  const deleteRegisteredUser = (user: RegisteredUser) => {
    if (deleteUserConfirmation !== "DELETE") {
    ...
    try {
      const updatedUsers = registeredUsers.filter(
        (u) => !(u.fullName === user.fullName && u.registeredAt === user.registeredAt),
      )
      localStorage.setItem("registeredUsers", JSON.stringify(updatedUsers))
      setRegisteredUsers(updatedUsers)
      setUserToDelete(null)
      setDeleteUserConfirmation("")
      setSuccess(`User ${user.fullName} has been deleted successfully`)
      setTimeout(() => setSuccess(""), 3000)
      setError("")
    } catch (error) {
                          ...
                          <TableCell>
                            <div className="flex gap-2">
                              ...
                              <AlertDialog>
                                ...
                                <AlertDialogContent>
                                  <AlertDialogHeader>
                                    <AlertDialogTitle>Delete Registered User</AlertDialogTitle>
                                    <AlertDialogDescription>
                                      This action cannot be undone. This will permanently delete the user{" "}
                                      <strong>{user.fullName}</strong> registered on{" "}
                                      <strong>{new Date(user.registeredAt).toLocaleString()}</strong>.
                                      {user.candidateNumber && (
                                        <>
                                          {" "}
                                          This user has candidate number <strong>{user.candidateNumber}</strong>.
                                        </>
                                      )}
                                    </AlertDialogDescription>
                                  </AlertDialogHeader>
                                  <div className="my-4">
                                    <Label htmlFor="deleteUserConfirmation">
                                  ...
                                  <AlertDialogFooter>
                                    ...
                                    <AlertDialogAction
                                      onClick={() => userToDelete && deleteRegisteredUser(userToDelete)}
                                      className="bg-red-600 hover:bg-red-700"
                                      disabled={deleteUserConfirmation !== "DELETE"}
                                    >
                                      Delete Permanently
                                    </AlertDialogAction>
                                  </AlertDialogFooter>
                              ...
                              <Button variant="outline" size="sm" onClick={() => router.push("/admin/test-selection")}>
                                Assign Tests
                              </Button>
                            </div>
                          </TableCell>

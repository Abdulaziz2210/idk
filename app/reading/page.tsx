...
export default function ReadingTest() {
  ...
  const [answers, setAnswers] = useState<Record<string, string>>({})
  const [isStarted, setIsStarted] = useState(false)
  const [timeRemaining, setTimeRemaining] = useState(1 * 60) // 60 minutes in seconds
  const [annotations, setAnnotations] = useState<any[]>([])
  const [currentTool, setCurrentTool] = useState<string | null>(null)
  ...
  useEffect(() => {
    if (isStarted) {
      ...
      localStorage.setItem("readingTimeRemaining", timeRemaining.toString())
      localStorage.setItem("readingLastSaveTime", Date.now().toString())

      const timer = setInterval(() => {
        setTimeRemaining((prev) => {
          const newTime = prev - 1
          localStorage.setItem("readingTimeRemaining", newTime.toString())
          localStorage.setItem("readingLastSaveTime", Date.now().toString())
  ...
  useEffect(() => {
    ...
    if (savedAnswers) {
      try {
        ...
        console.log("Loaded reading answers:", parsedAnswers)
      } catch (error) {
        console.error("Error parsing saved reading answers:", error)
        setAnswers({})
      }
    }
    ...
    // Check if test was already started and apply refresh penalty
    const testStarted = localStorage.getItem("readingTestStarted")
    if (testStarted === "true") {
      setIsStarted(true)

      // Apply refresh penalty
      const savedTime = localStorage.getItem("readingTimeRemaining")
      const lastSaveTime = localStorage.getItem("readingLastSaveTime")
      const refreshCount = Number.parseInt(localStorage.getItem("readingRefreshCount") || "0") + 1

      setRefreshPenalties(refreshCount)
      localStorage.setItem("readingRefreshCount", refreshCount.toString())
  ...
  useEffect(() => {
    if (Object.keys(answers).length > 0) {
      localStorage.setItem("readingAnswers", JSON.stringify(answers))
      console.log("Saved reading answers:", answers)
    }
  }, [answers])
  ...
  const handleAnswerChange = (questionId: string, value: string) => {
    ...
    console.log(`Updated reading answer ${questionId}:`, value)
  }
      ...
      questions: [
        {
          ...
          statements: [
            { id: "14", text: "14. Believed that the traditional approach was flawed." },
            { id: "15", text: "15. Proposed a new theoretical framework." },
            { id: "16", text: "16. Conducted the first empirical study on the subject." },
            { id: "17", text: "17. Criticized the methodology of previous studies." },
            { id: "18", text: "18. Found contradictory evidence to the established theory." },
            { id: "19", text: "19. Suggested practical applications for the research findings." },
            { id: "20", text: "20. Advocated for interdisciplinary collaboration." },
          ],
        },
        {
          id: "21",
      ...
      questions: [
        {
          ...
          paragraphs: [
            { id: "27", text: "A. The first paragraph of the passage." },
            { id: "28", text: "B. The second paragraph of the passage." },
            { id: "29", text: "C. The third paragraph of the passage." },
            { id: "30", text: "D. The fourth paragraph of the passage." },
            { id: "31", text: "E. The fifth paragraph of the passage." },
            { id: "32", text: "F. The sixth paragraph of the passage." },
            { id: "33", text: "G. The seventh paragraph of the passage." },
          ],
        },
        {
          id: "34",
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="flex justify-center p-4 border-t border-gray-200 bg-white">
        <p className="text-sm text-gray-500">Use the passage buttons above to navigate between passages</p>
      </div>
    </div>
  )
}
